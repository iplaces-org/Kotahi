/**
 * TitlesInput — repeatable DataCite additional-titles rows (subtitle,
 * alternative, translated). The MAIN title stays in the form's own $title
 * field and serializes with no titleType; these rows are appended after it.
 *
 * Each row = one DataCite 4.7 title:
 *   { id, title, titleType, lang }   (lang = BCP-47 tag, e.g. fr, ty)
 *
...
import { getTitleFields } from '../../../../shared/dataciteFieldDefinitions'
...
const TitlesInput = ({ onChange, value }) => {
  const cleanedVal = Array.isArray(value) ? value : []
  const fields = getTitleFields()

  if (value && !Array.isArray(value))
    // eslint-disable-next-line no-console
    console.error('Illegal TitlesInput value:', value)
